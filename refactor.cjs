...
const filePath = path.join('c:', 'Users', 'LENOVO', 'Documents', 'KTG old', 'src', 'components', 'Step3HitungHpp.tsx');
let content = fs.readFileSync(filePath, 'utf8');

// 1. State replacements
const oldState = `  // Packaging prices modal local state
  const [showPackagingModal, setShowPackagingModal] = useState(false);
  const [localPrices, setLocalPrices] = useState({ ...packagingPrices });
  const [saveAlert, setSaveAlert] = useState(false);`;

const newState = `  // Zone A: Material Input State
...
const oldHandlers = `  // Handle save packaging prices
...
const newHandlers = `  // Zone A Handlers
  const handleAddCustomMaterial = () => {
    if (!newMatName.trim()) return;
    const price = parseFloat(newMatPrice) || 0;
    const qty = parseFloat(newMatQty) || 0;
...
const layoutStartIdx = content.indexOf("  // --------------------------------------------------------------------------\n  // 1. Password Lock Gate Screen");
if (layoutStartIdx === -1) {
    console.error("Could not find start of layout block");
    process.exit(1);
}

const preLayout = content.substring(0, layoutStartIdx);
let postLayoutRaw = content.substring(layoutStartIdx);

// Remove the packaging modal from the end
const modalStartIdx = postLayoutRaw.indexOf("      {/* Modal Settings: Harga Bahan Kemasan");
if (modalStartIdx !== -1) {
    const modalEndIdx = postLayoutRaw.indexOf("      {/* Modal: Ganti Password HPP */}", modalStartIdx);
    if (modalEndIdx !== -1) {
        postLayoutRaw = postLayoutRaw.substring(0, modalStartIdx) + postLayoutRaw.substring(modalEndIdx);
    }
}

// Modify the Biaya Kemasan Button (Exclusive inside HPP!) -> remove it
const btnStartIdx = postLayoutRaw.indexOf("            {/* Biaya Kemasan Button (Exclusive inside HPP!) */}");
if (btnStartIdx !== -1) {
    const btnEndIdx = postLayoutRaw.indexOf("            {/* Excel Button */}", btnStartIdx);
    if (btnEndIdx !== -1) {
        postLayoutRaw = postLayoutRaw.substring(0, btnStartIdx) + postLayoutRaw.substring(btnEndIdx);
    }
}

const zoneAUI = `      {/* ZONE A: MATERIAL INPUT FORM (UNLOCKED) */}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-4 border-t border-slate-800">
          <div className="bg-slate-950 p-3 rounded-xl border border-slate-800">
            ...
            <input
              ...
              className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2.5 text-emerald-400 font-mono font-bold text-base focus-ring tabular-nums"
            />
          </div>
          <div className="bg-slate-950 p-3 rounded-xl border border-slate-800">
...
const lockScreenStartStr = '    return (\n      <div className="max-w-md mx-auto py-8 sm:py-12 px-4 animate-in fade-in">';
const lockScreenStartIdx = postLayoutRaw.indexOf(lockScreenStartStr);
const lockScreenEndIdx = postLayoutRaw.indexOf("    );\n  }\n\n  // --------------------------------------------------------------------------\n  // 2. Unlocked Full HPP Financial Screen");

if (lockScreenStartIdx === -1 || lockScreenEndIdx === -1) {
    console.error("Could not find lock screen");
    process.exit(1);
}

let lockScreenCode = postLayoutRaw.substring(lockScreenStartIdx + 13, lockScreenEndIdx);

const unlockedStartStr = '  return (\n    <div className="space-y-4 sm:space-y-6 max-w-4xl mx-auto">';
const unlockedStartIdx = postLayoutRaw.indexOf(unlockedStartStr);

if (unlockedStartIdx === -1) {
    console.error("Could not find unlocked screen");
    process.exit(1);
}

let unlockedCode = postLayoutRaw.substring(unlockedStartIdx + unlockedStartStr.length);
unlockedCode = unlockedCode.substring(0, unlockedCode.lastIndexOf("    </div>\n  );\n};\n"));

const newLayout = `  return (
    <div className="space-y-4 sm:space-y-6 max-w-4xl mx-auto">
\${zoneAUI}
      {/* ZONE B: HPP RESULTS (LOCKED) */}
      {!isHppUnlocked ? (\${lockScreenCode}      ) : (
        <>
\${unlockedCode}        </>
      )}
    </div>
